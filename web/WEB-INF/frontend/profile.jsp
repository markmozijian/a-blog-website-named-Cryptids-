        ...
        <div class="sign-up-form">
            <div id="sign-up-password">
                ...
                <div class="center-pic">
                    <img src="assets/avatars/${view_user.avatar}" class="center-pic">
                </div>
                </c:if>
                <div class="profile-container">
                    Username:<p>${view_user.username}</p>
                    <br>
                    <c:if test="${user.ID == view_user.ID}">
                    Email:<p>${view_user.email}</p>
                    <br>
                    </c:if>
                    Real Name:<p>${view_user.realName}</p>
                    <br>
                    Date of Birth:<p>${view_user.dateOfBirth}</p>
                    <br>
                    Country <p>${view_user.country}</p>
                    <br>
                    Bio:<p>${view_user.bio}</p>
                    <br>
                    <c:if test="${user.ID < 0}">
                        <a id="destroybtn" class="btn btn-danger" href="./murder?victimID=${view_user.ID}">Destroy User</a>
                    </c:if>
                    <c:if test="${user.ID == view_user.ID}">
                        <a class="btn btn-danger" href="./suicide">Self Destruct</a>
                    </c:if>
                </div>
            </div>
